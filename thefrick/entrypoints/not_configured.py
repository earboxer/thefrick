...
def _get_not_configured_usage_tracker_path():
    ...
    return Path(gettempdir()).joinpath(u'thefrick.last_not_configured_run_{}'.format(
        getpass.getuser(),
    ))
...
def _is_second_run():
    """Returns `True` when we know that `frick` called second time."""
    ...
    return (_get_previous_command() == 'frick' or
...
def main():
    ...
    It'll be only visible when user type frick and when alias isn't configured.
