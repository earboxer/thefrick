...
def _skip_old_lines(log_file):
    size = os.path.getsize(os.environ['THEFRICK_OUTPUT_LOG'])
...
def get_output(script):
    ...
    if 'THEFRICK_OUTPUT_LOG' not in os.environ:
    ...
    if const.USER_COMMAND_MARK not in os.environ.get('PS1', ''):
        logs.warn(
            ...
            "that PS1 is not changed after The Frick alias initialization")
        return None

    try:
        with logs.debug_time(u'Read output from log'):
            fd = os.open(os.environ['THEFRICK_OUTPUT_LOG'], os.O_RDONLY)
