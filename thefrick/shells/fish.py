...
class Fish(Generic):
    ...
    def _get_overridden_aliases(self):
        overridden = os.environ.get('THEFRICK_OVERRIDDEN_ALIASES',
    ...
    def app_alias(self, alias_name):
        if settings.alter_history:
            alter_history = ('    builtin history delete --exact'
                             ' --case-sensitive -- $fricked_up_command\n'
                             '    builtin history merge ^ /dev/null\n')
        else:
            alter_history = ''
        # It is VERY important to have the variables declared WITHIN the alias
        return ('function {0} -d "Correct your previous console command"\n'
                '  set -l fricked_up_command $history[1]\n'
                '  env TF_SHELL=fish TF_ALIAS={0} PYTHONIOENCODING=utf-8'
                ' thefrick $fricked_up_command {2} $argv | read -l unfricked_command\n'
                '  if [ "$unfricked_command" != "" ]\n'
                '    eval $unfricked_command\n{1}'
                '  end\n'
    ...
    def how_to_configure(self):
        return self._create_shell_configuration(
            content=u"thefrick --alias | source",
