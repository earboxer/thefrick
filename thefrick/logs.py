...
def how_to_configure_alias(configuration_details):
    print(u"Seems like {bold}frick{reset} alias isn't configured!".format(
        bold=color(colorama.Style.BRIGHT),
        reset=color(colorama.Style.RESET_ALL)))
    ...
    if configuration_details:
        ...
        if configuration_details.can_configure_automatically:
            print(
                u"Or run {bold}frick{reset} a second time to configure"
                u" it automatically.".format(
                    bold=color(colorama.Style.BRIGHT),
                    reset=color(colorama.Style.RESET_ALL)))

    print(u'More details - https://github.com/nvbn/thefrick#manual-installation')


def already_configured(configuration_details):
    print(
        u"Seems like {bold}frick{reset} alias already configured!\n"
...
def configured_successfully(configuration_details):
    print(
        u"{bold}frick{reset} alias configured successfully!\n"
        u"For applying changes run {bold}{reload}{reset}"
        u" or restart your shell.".format(
            bold=color(colorama.Style.BRIGHT),
            reset=color(colorama.Style.RESET_ALL),
...
def version(thefrick_version, python_version, shell_info):
    sys.stderr.write(
        u'The Frick {} using Python {} and {}\n'.format(thefrick_version,
                                                       python_version,
                                                       shell_info))
