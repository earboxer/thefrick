...
def get_all_executables():
    ...
    tf_alias = get_alias()
    tf_entry_points = ['thefrick', 'frick']
...
class Cache(object):
    ...
    def _setup_db(self):
        ...
        cache_path = Path(cache_dir).joinpath('thefrick').as_posix()
...
def get_installation_info():
    ...
    return pkg_resources.require('thefrick')[0]


def get_alias():
    return os.environ.get('TF_ALIAS', 'frick')


@memoize
def get_valid_history_without_current(command):
    def _not_corrected(history, tf_alias):
        """Returns all lines from history except that comes before `frick`."""
        previous = None
        for line in history:
