...
from thefrick.exceptions import EmptyCommand
...
class TestCorrectedCommand(object):
    ...
    @pytest.mark.parametrize('script, printed, override_settings', [
        ...
        ('git brunch',
         "git brunch || frick --repeat --force-command 'git brunch'",
         {'repeat': True, 'debug': False}),
        ('git brunch',
         "git brunch || frick --repeat --debug --force-command 'git brunch'",
...
class TestCommand(object):
    ...
    def Popen(self, monkeypatch):
        ...
        monkeypatch.setattr('thefrick.output_readers.rerun.Popen', Popen)
        return Popen

    @pytest.fixture(autouse=True)
    def prepare(self, monkeypatch):
        monkeypatch.setattr('thefrick.output_readers.rerun._wait_output',
                            lambda *_: True)
