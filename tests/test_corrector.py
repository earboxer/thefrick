...
from thefrick import corrector, const
from thefrick.system import Path
from thefrick.types import Command
from thefrick.corrector import get_corrected_commands, organize_commands
...
class TestGetRules(object):
    ...
    def glob(self, mocker):
        results = {}
        mocker.patch('thefrick.system.Path.glob',
                     new_callable=lambda: lambda *_: results.pop('value', []))
        return lambda value: results.update({'value': value})

    @pytest.fixture(autouse=True)
    def load_source(self, monkeypatch):
        monkeypatch.setattr('thefrick.types.load_source',
                            lambda x, _: Rule(x))
...
def test_get_corrected_commands(mocker):
    ...
    rules = [Rule(match=lambda _: False),
             ...
             Rule(match=lambda _: True,
                  ...
                  priority=60)]
    mocker.patch('thefrick.corrector.get_rules', return_value=rules)
