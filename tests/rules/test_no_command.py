import pytest
from thefrick.rules.no_command import match, get_new_command
from thefrick.types import Command


@pytest.fixture(autouse=True)
def get_all_executables(mocker):
    mocker.patch('thefrick.rules.no_command.get_all_executables',
...
def history_without_current(mocker):
    return mocker.patch(
        'thefrick.rules.no_command.get_valid_history_without_current',
...
def test_match(mocker, script, output):
    mocker.patch('thefrick.rules.no_command.which', return_value=None)
...
def test_not_match(mocker, script, output, which):
    mocker.patch('thefrick.rules.no_command.which', return_value=which)
