import pytest
from thefrick.rules.scm_correction import match, get_new_command
from thefrick.types import Command


@pytest.fixture
def get_actual_scm_mock(mocker):
    return mocker.patch('thefrick.rules.scm_correction._get_actual_scm',
                        return_value=None)
...
@pytest.mark.parametrize('script, output, actual_scm', [
    ('git log', 'fatal: Not a git repository '
     ...
     'hg'),
    ('hg log', "abort: no repository found in '/home/nvbn/exp/thefrick' "
               "(.hg not found)!",
     'git')])
...
@pytest.mark.parametrize('script, output, actual_scm', [
    ...
    ('hg log', "abort: no repository found in '/home/nvbn/exp/thefrick' "
               "(.hg not found)!",
     None),
    ('not-scm log', "abort: no repository found in '/home/nvbn/exp/thefrick' "
                    "(.hg not found)!",
     'git')])
