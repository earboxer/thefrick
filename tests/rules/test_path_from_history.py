import pytest
from thefrick.rules.path_from_history import match, get_new_command
...
def history(mocker):
    return mocker.patch(
        'thefrick.rules.path_from_history.get_valid_history_without_current',
        return_value=['cd /opt/java', 'ls ~/work/project/'])


@pytest.fixture(autouse=True)
def path_exists(mocker):
    path_mock = mocker.patch('thefrick.rules.path_from_history.Path')
