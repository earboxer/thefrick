import pytest
from thefrick.rules.gradle_wrapper import match, get_new_command
from thefrick.types import Command


@pytest.fixture(autouse=True)
def exists(mocker):
    return mocker.patch('thefrick.rules.gradle_wrapper.os.path.isfile',
                        return_value=True)
...
def test_match(mocker, command):
    mocker.patch('thefrick.rules.gradle_wrapper.which', return_value=None)
...
def test_not_match(mocker, exists, command, gradlew, which):
    mocker.patch('thefrick.rules.gradle_wrapper.which', return_value=which)
