...
import pytest
from thefrick.entrypoints.alias import _get_alias
...
def test_get_alias(monkeypatch, mocker, py2,
    ...
    args = Mock(
        ...
        alias='frick', )
    mocker.patch('thefrick.entrypoints.alias.which', return_value=which)
    shell = Mock(app_alias=lambda _: 'app_alias',
                 instant_mode_alias=lambda _: 'instant_mode_alias')
    monkeypatch.setattr('thefrick.entrypoints.alias.shell', shell)
