...
from thefrick.shells.generic import ShellConfiguration
from thefrick.entrypoints.not_configured import main


@pytest.fixture(autouse=True)
def usage_tracker(mocker):
    return mocker.patch(
        'thefrick.entrypoints.not_configured._get_not_configured_usage_tracker_path',
...
def shell_pid(mocker):
    return mocker.patch('thefrick.entrypoints.not_configured._get_shell_pid',
                        new_callable=MagicMock)


@pytest.fixture(autouse=True)
def shell(mocker):
    shell = mocker.patch('thefrick.entrypoints.not_configured.shell',
                         new_callable=MagicMock)
    shell.get_history.return_value = []
    shell.how_to_configure.return_value = ShellConfiguration(
        content='eval $(thefrick --alias)',
        path='/tmp/.bashrc',
        reload='bash',
...
def shell_config(mocker):
    path_mock = mocker.patch('thefrick.entrypoints.not_configured.Path',
                             new_callable=MagicMock)
...
@pytest.fixture(autouse=True)
def logs(mocker):
    return mocker.patch('thefrick.entrypoints.not_configured.logs',
                        new_callable=MagicMock)
...
def test_on_run_after_other_commands(usage_tracker_io, shell_pid, shell, logs):
    shell_pid.return_value = 12
    shell.get_history.return_value = ['frick', 'ls']
...
def test_on_first_run_from_current_shell(usage_tracker_io, shell_pid,
                                         shell, logs):
    shell.get_history.return_value = ['frick']
    shell_pid.return_value = 12
    main()
    _assert_tracker_updated(usage_tracker_io, 12)
    logs.how_to_configure_alias.assert_called_once()
...
def test_when_already_configured(usage_tracker_io, shell_pid,
    ...
    shell_pid.return_value = 12
    _change_tracker(usage_tracker_io, 12)
    shell_config.read.return_value = 'eval $(thefrick --alias)'
    main()
...
def test_when_successfully_configured(usage_tracker_io, shell_pid,
    ...
    main()
    shell_config.write.assert_any_call('eval $(thefrick --alias)')
