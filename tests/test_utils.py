...
from thefrick.utils import default_settings, \
...
class TestGetCloseMatches(object):
    @patch('thefrick.utils.difflib_get_close_matches')
...
def get_aliases(mocker):
    mocker.patch('thefrick.shells.shell.get_aliases',
                 return_value=['vim', 'apt-get', 'fsck', 'frick'])
...
def test_get_all_executables():
    ...
    assert 'frick' not in all_callables
...
class TestCache(object):
    ...
    def shelve(self, mocker):
        ...
        mocker.patch('thefrick.utils.shelve.open', new_callable=lambda: _Shelve)
        return value

    @pytest.fixture(autouse=True)
    def enable_cache(self, monkeypatch, shelve):
        monkeypatch.setattr('thefrick.utils.cache.disabled', False)
        _cache._init_db()

    @pytest.fixture(autouse=True)
    def mtime(self, mocker):
        mocker.patch('thefrick.utils.os.path.getmtime', return_value=0)
    ...
    def key(self, monkeypatch):
        monkeypatch.setattr('thefrick.utils.Cache._get_key',
                            lambda *_: 'key')
        return 'key'
...
class TestGetValidHistoryWithoutCurrent(object):
    ...
    def history(self, mocker):
        mock = mocker.patch('thefrick.shells.shell.get_history')
        #  Passing as an argument causes `UnicodeDecodeError`
        #  with newer py.test and python 2.7
        mock.return_value = ['le cat', 'frick', 'ls cat',
                             'diff x', 'nocommand x', u'café ô']
        return mock

    @pytest.fixture(autouse=True)
    def alias(self, mocker):
        return mocker.patch('thefrick.utils.get_alias',
                            return_value='frick')
    ...
    def bins(self, mocker):
        ...
        return mocker.patch('thefrick.utils.Path', return_value=path_mock)

    @pytest.mark.parametrize('script, result', [
        ('le cat', ['ls cat', 'diff x', u'café ô']),
        ('diff x', ['ls cat', u'café ô']),
        ('frick', ['ls cat', 'diff x', u'café ô']),
