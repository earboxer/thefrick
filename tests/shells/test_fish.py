...
from thefrick.shells import Fish
...
class TestFish(object):
    ...
    def Popen(self, mocker):
        mock = mocker.patch('thefrick.shells.fish.Popen')
    ...
    @pytest.mark.parametrize('key, value', [
        ...
        ('THEFRICK_OVERRIDDEN_ALIASES', 'cut,git,sed'),
        ('THEFRICK_OVERRIDDEN_ALIASES', 'cut, git, sed'),
        ('THEFRICK_OVERRIDDEN_ALIASES', ' cut,\tgit,sed\n'),
        ('THEFRICK_OVERRIDDEN_ALIASES', '\ncut,\n\ngit,\tsed\r')])
    ...
    def test_app_alias(self, shell):
        assert 'function frick' in shell.app_alias('frick')
        assert 'function FRICK' in shell.app_alias('FRICK')
        assert 'thefrick' in shell.app_alias('frick')
        assert 'TF_SHELL=fish' in shell.app_alias('frick')
        assert 'TF_ALIAS=frick PYTHONIOENCODING' in shell.app_alias('frick')
        assert 'PYTHONIOENCODING=utf-8 thefrick' in shell.app_alias('frick')
        assert ARGUMENT_PLACEHOLDER in shell.app_alias('frick')

    def test_app_alias_alter_history(self, settings, shell):
        settings.alter_history = True
        assert 'builtin history delete' in shell.app_alias('FRICK')
        assert 'builtin history merge' in shell.app_alias('FRICK')
        settings.alter_history = False
        assert 'builtin history delete' not in shell.app_alias('FRICK')
        assert 'builtin history merge' not in shell.app_alias('FRICK')
    ...
    def test_put_to_history(self, entry, entry_utf8, builtins_open, mocker, shell):
        mocker.patch('thefrick.shells.fish.time', return_value=1430707243.3517463)
